// app/api/scheduling/admin/appointments/[id]/restore/route.ts
...
export async function POST(
  ...
  const res = await updateAppointmentStatus({
    ...
    allowedCurrent: ["canceled"],
    reason,
    allowRestore: true,
  });

  if (!res.ok) return NextResponse.json({ error: res.error }, { status: 409 });
  return NextResponse.json({ appointment: res.appointment, emailError: res.emailError });
}
