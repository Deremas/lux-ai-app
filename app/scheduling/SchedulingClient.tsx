...
const schedulingCopyFr = {
  ...
  step5: { title: "Etape 5 · Confirmation", body: "Verifiez les details avant de confirmer votre reservation.", meeting: "Session", sessionFallback: "Session", phoneCallTo: "Appel vers", meetingLinkLater: "Le lien de reunion sera envoye par e-mail apres confirmation.", time: "Horaire", booker: "Reservant", personalBooking: "Reservation personnelle", notes: "Notes", preferredTimezone: "Fuseau horaire prefere", payment: "Paiement", paymentRequired: "Paiement requis", paid: "Paiement : paye", checkoutHint: "Cliquez sur « Passer au paiement » pour payer dans Stripe avant que cette reservation puisse etre confirmee.", holdWindow: "Votre creneau selectionne est reserve jusqu'a {minutes} minutes pendant que vous finalisez le paiement Stripe.", approvalHint: "Si l'organisation exige une approbation, la reservation peut rester en attente jusqu'a validation par l'equipe.", paymentLinkHint: "Si la redirection echoue, utilisez le lien de paiement fourni par votre administrateur.", paymentStatus: "Statut du paiement", notifications: "Vous recevrez un e-mail de confirmation si les notifications sont activees.", selectTimeNotice: "Selectionnez un horaire pour verifier les details de votre reservation." },
...
const schedulingCopyDe = {
  ...
  step5: { title: "Schritt 5 · Bestatigung", body: "Prufen Sie die Details, bevor Sie Ihre Buchung bestatigen.", meeting: "Meeting", sessionFallback: "Session", phoneCallTo: "Telefonat an", meetingLinkLater: "Der Meeting-Link wird nach der Bestatigung per E-Mail gesendet.", time: "Zeit", booker: "Buchende Person", personalBooking: "Personliche Buchung", notes: "Notizen", preferredTimezone: "Bevorzugte Zeitzone", payment: "Zahlung", paymentRequired: "Zahlung erforderlich", paid: "Zahlung: bezahlt", checkoutHint: "Klicken Sie auf „Zur Zahlung“, um in Stripe zu zahlen, bevor diese Buchung bestatigt werden kann.", holdWindow: "Ihr gewahlter Termin bleibt bis zu {minutes} Minuten reserviert, wahrend Sie die Stripe-Zahlung abschliessen.", approvalHint: "Wenn die Organisation eine Freigabe verlangt, kann Ihre Buchung dennoch auf ausstehend bleiben, bis das Team zustimmt.", paymentLinkHint: "Falls die Weiterleitung fehlschlagt, verwenden Sie den von Ihrem Admin bereitgestellten Zahlungslink.", paymentStatus: "Zahlungsstatus", notifications: "Sie erhalten eine Bestatigungs-E-Mail, wenn Benachrichtigungen aktiviert sind.", selectTimeNotice: "Wahlen Sie eine Zeit, um die Buchungsdetails zu prufen." },
...
function interpolateMinutes(template: string, minutes: number | null) {
  if (minutes === null || !Number.isFinite(minutes)) return template;
...
export default function SchedulingClient(props: Props) {
  ...
  const [reservationHoldMinutes, setReservationHoldMinutes] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    fetch(url.toString(), { cache: "no-store" })
      ...
      .then(({ ok, data }) => {
        ...
        const holdMinutes =
          typeof data?.reservationHoldMinutes === "number" &&
          Number.isFinite(data.reservationHoldMinutes)
        ...
        setReservationHoldMinutes(holdMinutes);
                    ...
                    <>
                      ...
                      {!bookingSummary && requiresCheckout && (
                        <div className="mt-3 space-y-2">
                          ...
                          {reservationHoldMinutes !== null && (
                            <p className="text-xs text-emerald-700">
                              {interpolateMinutes(
                                copy.step5.holdWindow,
